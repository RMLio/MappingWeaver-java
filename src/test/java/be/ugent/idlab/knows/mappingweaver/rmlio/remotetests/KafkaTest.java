package be.ugent.idlab.knows.mappingweaver.rmlio.remotetests;
...
public class KafkaTest extends TestCore {
    ...
    public void csvTest() throws Exception {
        ...
        String file = FileUtils.readFileToString(new File("src/test/resources/rmlio/test-cases/integrations/kafka/csv/input.csv"), Charset.defaultCharset());
        ...
        String planTTL = Files.readString(Paths.get("src/test/resources/rmlio/test-cases/integrations/kafka/csv/mapping.ttl"));
    ...
    public void jsonTest() throws Exception {
        String file = FileUtils.readFileToString(new File("src/test/resources/rmlio/test-cases/integrations/kafka/json/input.json"), Charset.defaultCharset());
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(this.producerProperties())) {
            var metadata = producer.send(new ProducerRecord<>("demo", "input.json", file)).get();
            System.out.println(metadata);
        }

        String planTTL = Files.readString(Paths.get("src/test/resources/rmlio/test-cases/integrations/kafka/json/mapping.ttl"));
        planTTL = planTTL.replace("localhost:9092", kafka.getBootstrapServers());
        String planJson = ITranslator.getInstance().translate_to_document(planTTL);
    ...
    public void xmlTest() throws Exception {
        String file = FileUtils.readFileToString(new File("src/test/resources/rmlio/test-cases/integrations/kafka/xml/input.xml"), Charset.defaultCharset());
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(this.producerProperties())) {
            producer.send(new ProducerRecord<>("demo", file, file)).get();
        }

        String planTTL = Files.readString(Paths.get("src/test/resources/rmlio/test-cases/integrations/kafka/xml/mapping.ttl"));
        planTTL = planTTL.replace("localhost:9092", kafka.getBootstrapServers());
        String planJson = ITranslator.getInstance().translate_to_document(planTTL);
