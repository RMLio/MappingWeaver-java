...
### Fixed
- A function producing several values is applied to every one of them, wherever it sits. A split in an object map used to yield only its first value: the values were dropped at three points, each keeping a single value where several were meant to travel. `FnOParameter` now hands a function all of a parameter's values and the function is run once per combination, `FnOFunction` and `LiteralTypeFunction` build a term per value, and AMO's `ExtendOperator` gives a record per value. Splitting `"read write"` now yields a triple for `read` and one for `write`.
- A value produced by a multi-valued function is a plain string instead of carrying the datatype of the collection the function declares it returns (`grel:string_split` declares `grel:output_array`, which surfaced as `^^rdf:List` on each value).
